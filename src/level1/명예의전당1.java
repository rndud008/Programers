...
public class 명예의전당1 {
    ...
    // 매일 출연한 가수의 점수가 지금까지 출연 가수들의
    // 점수 중 상위 k번째 이내이면 해당 가수의 점수를
    // 명예의 전당이라는 목록에 올려 기념합니다.
    // 즉 프로그램 시작 이후 초기에 k일까지는
    // 모든 출연 가수의 점수가 명예의 전당에 오르게 됩니다.
    // k일 다음부터는 출연 가수의 점수가 기존의 명예의 전당
    // 목록의 k번째 순위의 가수 점수보다 더 높으면,
    ...
    public int[] solution(int k, int[] score) {
        ...
        for (int i = 0; i < score.length; i++) {
            ...
            if (hallOfFame.size() > k) {
                hallOfFame.remove(hallOfFame.size() - 1); // 최하위 점수 제거
            }

            answer[i] = hallOfFame.get(hallOfFame.size() - 1); // 현재 명예의 전당 최하위 점수
        }
