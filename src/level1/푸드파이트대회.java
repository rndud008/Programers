...
public class 푸드파이트대회 {
    ...
    // 이때, 대회의 공정성을 위해 두 선수가 먹는
    // 음식의 종류와 양이 같아야 하며,
    // 음식을 먹는 순서도 같아야 합니다.
    // 또한, 이번 대회부터는 칼로리가 낮은 음식을
    // 먼저 먹을 수 있게 배치하여
    ...
    // 2번 음식을 4개, 3번 음식을 6개 준비했으며,
    // 물을 편의상 0번 음식이라고 칭한다면,
    // 두 선수는 1번 음식 1개, 2번 음식 2개,
    // 3번 음식 3개씩을 먹게 되므로 음식의 배치는
    // "1223330333221"이 됩니다.
    // 따라서 1번 음식 1개는 대회에 사용하지 못합니다.
    // 수웅이가 준비한 음식의 양을 칼로리가 적은 순서대로
    // 나타내는 정수 배열 food가 주어졌을 때,
    // 대회를 위한 음식의 배치를 나타내는
    ...
    public String solution(int[] food) {
        ...
        for (int i = 0; i < food.length-1; i++) {
            int count = food[i+1]/2;
            if (count !=0){
                answer.append(String.valueOf(i+1).repeat(count));
            }
        }
        StringBuilder builder = new StringBuilder(answer).reverse();

        return answer.append("0").append(builder).toString();
    }
}
